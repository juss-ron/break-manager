...
async function apiCall(endpoint, method = 'GET', body = null) {
    ...
    // Try to parse JSON, even if response.ok is false, to get error message
    // If it fails, the catch block will throw a generic API Error
    let data;
    try {
        data = await response.json();
    } catch (e) {
        if (!response.ok) {
            // This handles the 'Unexpected token <' error by forcing a generic error message
            throw new Error(`API returned an unexpected response format (Status: ${response.status})`);
        }
...
async function checkSession() {
    ...
    if (token) {
        try {
            ...
            try {
                // NOTE: All API calls here should be checked for the /api/ prefix if added in the future
                const teamData = await apiCall('/api/teams/current'); 
                currentTeam = teamData; // Team with included users
                const decodedToken = parseJwt(token);
                currentUser = currentTeam.Users.find(u => u.id === decodedToken.id); // Find user from included users
                ...
                // 404 means no team, or a different API error
                console.error('CheckSession error:', err.message);
                
                // We need to at least get the user name. 
                const decoded = parseJwt(token);
                // Fallback user object
                currentUser = { name: decoded.email ? decoded.email.split('@')[0] : 'User', ...decoded }; 
                ...
                if (page.includes('enterteam.html') && elements.userName) {
                    elements.userName.textContent = 'User ' + (currentUser.email || currentUser.name); // Simplified
                }
            }
...
async function updateDashboard() {
    ...
    try {
        // NOTE: Check all apiCall usages for /api/ prefix if required
        const teamData = await apiCall('/api/teams/current');
...
async function createTeam() {
    ...
    try {
        // NOTE: Check all apiCall usages for /api/ prefix if required
        const data = await apiCall('/api/teams', 'POST', { name: teamName });
...
async function joinTeam() {
    ...
    try {
        // NOTE: Check all apiCall usages for /api/ prefix if required
        const data = await apiCall('/api/teams/join', 'POST', { code });
        window.location.href = 'dashboard.html';
    } catch (error) {
...
async function startBreak() {
    try {
        // NOTE: Check all apiCall usages for /api/ prefix if required
        await apiCall('/api/timer/breaks/start', 'POST');
        updateDashboard();
    } catch (e) {
        alert(e.message);
    }
...
async function endBreak() {
    try {
        // NOTE: Check all apiCall usages for /api/ prefix if required
        await apiCall('/api/timer/breaks/end', 'POST');
        if (timerInterval) clearInterval(timerInterval);
        updateDashboard();
    } catch (e) {
        alert(e.message);
...
function setupEventListeners() {
    ...
    if (elements.saveTimeBtn) {
        elements.saveTimeBtn.addEventListener('click', async () => {
            ...
            try {
                // NOTE: Check all apiCall usages for /api/ prefix if required
                await apiCall('/api/timer/set', 'POST', { duration: totalSeconds });
                elements.setTimeModal.classList.remove('active');
                updateDashboard();
            } catch (e) {
                alert(e.message);
...
async function toggleHackathonTimer(start) {
    try {
        // NOTE: Check all apiCall usages for /api/ prefix if required
        await apiCall('/api/timer/status', 'POST', { isRunning: start });
...
async function deleteTeam() {
    ...
    if (confirm('Are you sure you want to delete this team? This action cannot be undone.')) {
        try {
            // NOTE: Check all apiCall usages for /api/ prefix if required
            await apiCall(`/api/teams/${currentTeam.id}`, 'DELETE');
            logout();
        } catch (e) {
            alert(e.message);
        }
